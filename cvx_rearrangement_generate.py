...
def load_canonical_mesh(data: dict) -> trimesh.Trimesh:
    ...
    return trimesh.Trimesh(vertices.cpu().float(), faces.cpu().long())


def transform2view_center(points: torch.Tensor, dist: float, elev: float, azim: float):
    points = points[None].cuda()
    dist = torch.tensor(dist).view(1, 1).cuda()
    elev = torch.tensor(elev).view(1, 1).cuda()
    azim = torch.tensor(azim).view(1, 1).cuda()
...
def generate():
    ...
    for i in tqdm(range(args.generate_size // 20)):
        instance_path = os.path.join(args.generate_root, '%06d' % i)
        ...
        for j in range(20):
            ...
            mesh = TriangleMesh.from_tensors(canonical2view(final_mesh.vertices, dist, elev, azim).cpu(),
                                             final_mesh.faces.cpu())

            rgb = to_pil(rgb)
            mask = to_pil(mask)

            rgb.save(os.path.join(instance_path, '%02d_rgb.png'% j))
            mask.save(os.path.join(instance_path, '%02d_mask.png' % j))
            mesh.save_mesh(os.path.join(instance_path, '%02d_mesh.obj' % j))
            json_data = json.dumps({'dist': dist, 'elev': elev, 'azim': azim})
            with open(os.path.join(instance_path, 'view_pose_%02d.json' % j), 'w') as f:
                f.write(json_data)
                f.close()
